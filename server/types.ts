...
export type SatelliteObservabilityStatus =
  | "observable"
  | "partially_observable"
  | "cloud_obscured"
  | "insufficient_valid_pixels"
  | "temporal_mismatch"
  | "not_suitable_for_event_type"
  | "not_suitable_for_resolution"
  | "provider_unavailable";

export type SatelliteAssessmentResult =
  | "potentially_consistent"
  | "possible_surface_change"
...
export interface SatelliteProductMedia {
  storagePath: string;
  cloudUri?: string;
  displayUrl: string;
...
export interface SatelliteObservability {
  status: SatelliteObservabilityStatus;
  score: number;
  localCloudPercent?: number;
  validPixelPercent?: number;
...
export interface SatelliteContextAssessment {
  result: SatelliteAssessmentResult;
  confidence: number;
  source: "deterministic" | "gemini_multimodal";
  observability: "observable" | "partially_observable" | "not_observable";
  eventSuitability: "suitable" | "partially_suitable" | "not_suitable";
  citizenPhotoSignals: string[];
  satelliteSignals: string[];
  surfaceChangeSignals: string[];
  contradictorySignals: string[];
  temporalConsistency: "relevant" | "weak" | "mismatched";
  spatialContext: "potentially_consistent" | "unclear" | "contradictory";
  explanation: string;
  limitations: string[];
}
...
export interface SatelliteComparison {
  baselineNBR?: number;
  comparisonNBR?: number;
  deltaNBR?: number;
  baselineBSI?: number;
  comparisonBSI?: number;
  deltaBSI?: number;
  baselineAot?: number;
  comparisonAot?: number;
  validPixelPercent: number;
}
...
export interface SatelliteEvidence {
  ...
  productMedia?: Partial<Record<keyof SatelliteEvidenceThumbnail, SatelliteProductMedia>>;
  ...
  previousFailures?: Array<{ code?: string; message: string; at: string }>;
}
...
export interface MediaEvidence {
  ...
  publicUrl?: string;
  displayUrl?: string;
  storageProvider?: "firebase_storage" | "local_dev";
  mimeType: string;
  sizeBytes: number;
  sha256Hash: string;
  uploadedAt: string;
...
export interface MeasurementEligibility { displayAsCurrentMeasurement: boolean; usableForLocalContext: boolean; usableForClusterStatistic: boolean; usableForAqi: boolean; reasons: string[]; }
...
export interface AirQualityWarning {
  code: AirQualityWarningCode;
  message: string;
  provider?: AirQualityProvider;
  pollutant?: CpcbPollutantCode;
}

export interface NormalizedPollutantReading {
  pollutant: CpcbPollutantCode;
  value?: number;
  unit: string;
  provider?: AirQualityProvider;
...
export interface PollutantSourceBundle {
  pollutant: CpcbPollutantCode;
  cpcb?: NormalizedPollutantReading;
  openaq?: NormalizedPollutantReading;
  selectedCurrent?: NormalizedPollutantReading;
  samePhysicalStation: boolean;
  selectionReason?: string;
  conflict?: { detected: boolean; absoluteDifference?: number; relativeDifferencePercent?: number; reason?: string };
}

export interface AirQualityStation {
  id: string;
  name: string;
  provider: AirQualityProvider | "matched";
  lat: number;
  lng: number;
  city?: string;
  state?: string;
  ...
  lastUpdate?: string;
  freshness?: "fresh" | "usable" | "stale" | "expired" | "unknown";
  pollutants: Partial<Record<CpcbPollutantCode, NormalizedPollutantReading>>;
  providers?: AirQualityProvider[];
  attribution?: string;
  license?: string;
  sourceReadings?: { cpcb?: Partial<Record<CpcbPollutantCode, NormalizedPollutantReading>>; openaq?: Partial<Record<CpcbPollutantCode, NormalizedPollutantReading>>; };
  sourceBundles?: Partial<Record<CpcbPollutantCode, PollutantSourceBundle>>;
}
...
export interface AirQualityMapPoint {
  ...
  metricDetails?: Partial<Record<AirQualityMapMetric, { value: number; unit: string; provider: AirQualityProvider; measuredAt?: string; ageHours?: number; freshness?: string; valueKind?: MeasurementValueKind; aggregationPeriod?: AggregationPeriod; aggregationPeriodVerified?: boolean; currentMapEligible: boolean; aqiEligible: boolean; warnings: string[] }>>;
    aqiQuality?: AirQualityAqiQuality;
    aqiStatus?: AirQualityMapAqi;
    attribution?: string;
    stale?: boolean;
...
export interface AirQualityMapResponse {
  ...
  warnings?: AirQualityWarning[];
  completeness?: { cpcbComplete: boolean; openAqMetadataComplete: boolean; openAqLatestComplete: boolean; nationalMapComplete: boolean };
...
export interface PollutionReport {
  ...
  cleanupProof?: { afterMedia: MediaEvidence; submittedAt: string; uploaderId: string; locality?: string; lat?: number; lng?: number; gpsAccuracy?: number; actionTaken?: string; note?: string; cleanupPercentage?: number; locationMatch?: boolean; aiConfidence?: number; remainingPollution?: string; summary?: string };
  actionTaken?: string;
...
export interface MeteorologicalProvider {
  name: string;
  getCurrentConditions(latitude: number, longitude: number): Promise<MeteorologicalContext>;
  getHourlyForecast(latitude: number, longitude: number, horizonHours?: number): Promise<{
    context: MeteorologicalContext;
    hourlyForecast: HourlyForecastPoint[];
  }>;
}
