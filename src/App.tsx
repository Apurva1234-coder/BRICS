...
export type Route = "map" | "federation" | "federation-propagation" | "federation-corridors" | "federation-regulatory" | "capture" | "my-reports" | "leaderboard" | "ngo" | "result" | "admin";
...
const navItems = [
  ...
  { route: "federation" as const, label: "Federation",       icon: Globe, children: [
    { route: "federation-propagation" as const, label: "Cross-Border Propagation" },
    { route: "federation-corridors" as const, label: "Economic Corridors" },
    { route: "federation-regulatory" as const, label: "Regulatory Coordination" },
  ] },
...
function routeFromLocation(): Route {
  ...
  if (["map", "federation", "federation-propagation", "federation-corridors", "federation-regulatory", "capture", "my-reports", "leaderboard", "ngo", "result", "admin"].includes(hash)) return hash as Route;
  return "map";
}
...
export default function App() {
  ...
  <AppShell navItems={navItems} activeRoute={route} onNavigate={navigate} stats={stats} onClearLocalDemoData={clearDemoData}>
      <Suspense fallback={<RouteLoading />}>
        ...
        {(route === "federation" || route === "federation-propagation") && <BricsFederationPanel section="propagation" />}
        {route === "federation-corridors" && <BricsFederationPanel section="corridors" />}
        {route === "federation-regulatory" && <BricsFederationPanel section="regulatory" />}
        {route === "capture" && (
          <ReportPage
