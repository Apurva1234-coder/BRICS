...
export function useRouteAccessibility(route: Route) {
  ...
  useEffect(() => {
    const names: Record<Route, string> = { map: t("nav.map"), federation: "BRICS Federation", "federation-propagation": "Cross-Border Propagation", "federation-corridors": "Economic Corridors", "federation-regulatory": "Regulatory Coordination", capture: t("nav.capture"), "my-reports": t("nav.myReports"), leaderboard: "Leaderboard", ngo: t("nav.municipal"), result: t("report.ticket"), admin: t("nav.officer") };
