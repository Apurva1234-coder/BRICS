...
export function SituationRail({
  ...
  const priorityCount = (priority: string) => situations.filter((s) => s.priority.toLowerCase() === priority.toLowerCase()).length;
        ...
        <div className="flex gap-1.5 overflow-x-auto pb-0.5 hide-scrollbar">
          {FILTERS.map((f) => (
            <button
              ...
              className={`filter-chip !px-2.5 !py-1 text-xs inline-flex items-center gap-1.5 flex-shrink-0 whitespace-nowrap ${activeFilter === f ? "filter-chip-active" : ""}`}
            >
              <span>{filterLabel(f)}</span>
              {f !== "All" && <span className="text-[10px] font-bold opacity-75">{priorityCount(f)}</span>}
            </button>
          ))}
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-3 py-3 space-y-2 hide-scrollbar">
        {filtered.length > 0 ? (
          filtered.map((situation) => {
            const latestReport = reports.filter((report) => situation.reportIds.includes(report.id)).sort((a, b) => Date.parse(b.createdAt) - Date.parse(a.createdAt))[0];
            const pm25Reading = latestReport?.airQuality.readings?.pm25?.value ?? latestReport?.airQuality.pollutants?.pm2_5;
            return (
            <SituationListItem
              key={situation.id}
              situation={situation}
              pm25={typeof pm25Reading === "number" ? pm25Reading : undefined}
