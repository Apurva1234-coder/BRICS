...
export function LocalLeafletMap({
  ...
  return (
    <div ref={mapOverlayRef} className="relative flex h-full min-h-0 w-full flex-1 flex-col">
      ...
      <div className="flex h-full min-h-0 w-full flex-1 flex-col bg-slate-950">
        <MapContainer
        ...
        {/* View mode menu */}
        <div data-map-mode-selector className="pointer-events-auto absolute left-1/2 top-3.5 z-[400] w-[calc(100vw-2rem)] max-w-max -translate-x-1/2 overflow-x-auto [scrollbar-width:none]">
          <nav aria-label="Map perspective" className="flex w-max flex-nowrap items-center gap-1 rounded-xl border border-white/10 bg-slate-950/90 p-1 shadow-2xl backdrop-blur-md">
            {mapModes.map((item) => {
              const active = mode === item.key;
              return (
                <button
                  key={item.key}
                  className={`shrink-0 whitespace-nowrap rounded-lg px-3 py-1.5 text-xs font-bold tracking-wide transition-all
                    ${
                      active
